function [x, u, cost, v,delta_u] = SASS(y, d, fc, K, lam, u_init)
...
[A, B, B1, D] = Lowpass(d, fc, N, K); % Banded filter matrices
...
while not(last_iter)
    ...
    delta_u(iter) = max(abs( u - old_u )) / (max(abs(old_u))+eps);
    
    old_u = u;
     
    if (delta_u(end) <= TOL_STOP) || (iter >= MAX_ITER)
        last_iter = true;
    end
