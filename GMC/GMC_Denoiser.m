...
% GMC Recovery of highpass components
lambda = 0.1; % Regularisation parameter
gamma = 0.8; % Convexity parameter

%{
Outputs:
hpf = GMC-sparse signal
loss = loss function 
%}

% GMC - GMC Algorithm
[hpf, ~, loss] = GMC(residual, A, AH, 1, lambda, gamma);
hpf = A(hpf);

figure;
plot(t,hpf,'b')
title("GMC Sparse Signal")

% GMC filtered signal
GMC_filtered = (lpf + hpf)'; % GMC filtered signal
...
title("Noisy Signal")
subplot(3,1,3)
stft(GMC_filtered,360,Window=hann(20),OverlapLength=19,FFTLength=2000,FrequencyRange="onesided");
title("GMC Filtered Signal")
...
Error = noisy_signal - GMC_filtered;
RMSE = sqrt(mean(Error.^2))

% SNR (Signal to noise ratio)
Normalize = min(abs(pure_signal));
SNR = snr(Normalize*GMC_filtered,noisy_signal) + " dB"
